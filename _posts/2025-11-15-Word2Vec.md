...
![Photo Skip-gram]({{ '/assets/lib/img/word2vec/skipgram.png' | relative_url }})
...
![Photo Skip-gram]({{ '/assets/lib/img/word2vec/pairide.png' | relative_url }})
...
Ici K est le nombre de **negativ samples** qu'on décide de mettre pour chaque sample dans l'entrainement (on prend généralement K entre 5 et 20, papier dde Word2Vec), c'est un hyper-paramètre du modèle, il est fixe durant tout l'entrainement.
...
Pour cela, on va déjà calculer les dérivées partielles par rapport à chaque composantes de la loss:
...
De plus, il faut noter que l'on shuffle les pairs, donc tout nos données d'entrainement pour que le modèle traite équitablement chaque pair (pas que la pair (king,charles) n'entraîne aucune mise à jour, car elle se retrouve à la fin où le learning rate est à sa valeur minimale).
...
On va créer une table immense où chaque position correspond à un mot. Le nombre de fois que ce mot est dans la table est proportionnel à sa pro babilité d'être tiré en tant que negativ sample.
...
Je normalise aussi $W_in$ avant de trouver les k plus proches voisins.
**window_size_max = 3; embed_dim = 300; negatives = 5; lr_initial = 0.025; epochs = 5; t = 1e-3**
