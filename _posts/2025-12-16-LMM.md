...
C'est pour quoi on s'intérèsse à l'infos brute: la statistique nommée t-value définie comme $\frac{\beta}{SE(\beta)}$ où $\beta$ est un paramètre de la régréssion. En LMM, on l'utilise car on suppose un paramètre continu, estimé avec une incertitude Son nom change dans le cas d'une LMM, car la loi de la t-value n'est pas connue, on l'appelle alors une statistique de Wald.

La t-value est plus difficile à interpréter, car elle nécéssite du contexte pour être interpréter. Elle est néanmoins plus informative que la p-value, celle-ci étant une traduction de la t-value en écrasant de d'information. Une p-value peut en effet être construite à partir de la t-value et du nombre de degré de liberté du modèle. Le soucis est que en LMM on ne connait pas la loi exacte de la statistique (t-value) sous H0, car on a des observations corrélées, des degrés de liberté flous et des variances aléatoires estimées. 
